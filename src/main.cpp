...
DataReal trueI(0);
DataReal trueU(0);
DataEdit limI(100,0,900,10);
DataEdit limU(1050,550,1600,10);
DataEdit status(0,0,1,1);
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("mode, out, voltage, curent");
...
if(status.getData()){
    ...
    default:
      mode = processUI(limU.getData(), limI.getData(), out, trueU.getData(),trueI.getData());
...
if (TimerPrev != TimerNew) { // переменная TimerNew меняет свое значение раз в 10 милисекунд.  Для того чтобы выявить что это произошло  мы сравниваем ее с запомненным ранее значением
    ...
    // опрашиваем кнопки каждые 10 мс
    //menu.key(); // клавиши можно опрашивать раз в 3 обновления экрана
